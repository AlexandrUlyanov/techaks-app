    ...
    <section className="container-main pb-4">
      <div className="grid gap-6 lg:grid-cols-[250px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 rounded-[1.75rem] bg-muted/25 p-5 md:p-8">
          ...
          {section === "notifications" && notificationDraft && <div className="space-y-6"><header><h2 className="text-xl font-black">Уведомления</h2><p className="mt-2 text-sm text-muted-foreground">Сервисные сообщения о заказах и безопасности нельзя полностью отключить.</p></header>{([['orderEmail','Заказы по email'],['orderPush','Заказы: push'],['orderInApp','Заказы в кабинете'],['marketingEmail','Новости и предложения по email'],['marketingPush','Новости: push'],['priceDropEmail','Снижение цены по email'],['priceDropPush','Снижение цены: push']] as const).map(([key,label])=><label key={key} className="flex min-h-12 items-center justify-between gap-4 border-b border-border/50 py-3 text-sm font-semibold"><span>{label}</span><input type="checkbox" checked={notificationDraft[key]} onChange={e=>setNotificationDraft(v=>v?({...v,[key]:e.target.checked}):v)}/></label>)}<Button className="rounded-full bg-[#05C3D4] text-black" onClick={()=>saveNotifications.mutate(notificationDraft as any)}>Сохранить настройки</Button></div>}
