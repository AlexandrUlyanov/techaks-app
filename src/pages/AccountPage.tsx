        ...
        <div className="container-main relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-3xl font-black uppercase font-heading tracking-tighter">{user?.fullName}</h1>
                <p className="text-muted-foreground font-bold uppercase tracking-widest text-[10px] mt-1">{user?.phone}</p>
              </div>
            </div>
            <Button 
              variant="outline" 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="flex items-center gap-4 mb-2">
              ...
              <h2 className="text-xl font-black uppercase font-heading tracking-tight">История заказов</h2>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="p-12 bg-card rounded-3xl border border-border text-center space-y-6">
                ...
                <p className="text-muted-foreground font-medium uppercase tracking-widest text-xs">У вас пока нет заказов</p>
                <Button variant="outline" className="h-12 px-10 border-border">В МАГАЗИН</Button>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex flex-wrap justify-between gap-6 mb-6">
                      <div className="space-y-1">
                        ...
                        <p className="text-lg font-black font-heading tracking-tight">#{order.id}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Статус</span>
                        <div className="flex items-center gap-2">
                          <div className="w-1.5 h-1.5 rounded-full bg-[#22c55e] animate-pulse" />
                          <p className="text-sm font-black uppercase tracking-tight text-[#22c55e]">{order.status}</p>
                        </div>
                      </div>
                      <div className="space-y-1">
                        <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Дата</span>
                        <p className="text-sm font-bold">{new Date(order.createdAt).toLocaleDateString("ru-RU")}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Сумма</span>
                        <p className="text-lg font-black text-[#05C3D4] font-heading">{formatPrice(order.totalPrice)}</p>
                      </div>
                    </div>
                    ...
                    <div className="pt-6 border-t border-border flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#05C3D4] opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                        ДЕТАЛИ <ChevronRight size={14} className="inline ml-1" />
                      </span>
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-6">
            <div className="p-8 bg-[#05C3D4] rounded-3xl text-black relative overflow-hidden group">
              <div className="relative z-10">
                <h3 className="text-xl font-black uppercase font-heading tracking-tight leading-none">ТЕХАКС <br /> ПРИВИЛЕГИИ</h3>
              ...
              <Star size={120} className="absolute -bottom-10 -right-10 text-black/10 transform rotate-12 group-hover:scale-110 transition-transform duration-700" />
            </div>

            <div className="p-8 bg-muted rounded-3xl border border-border">
              <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground mb-6">Поддержка</h3>
              <div className="space-y-4">
                <a href="tel:+79273750555" className="flex items-center justify-between text-sm font-bold hover:text-[#05C3D4] transition-colors">
                  <span>Позвонить нам</span>
                  <ChevronRight size={16} />
                </a>
                <Separator className="bg-border/50" />
                <a href="https://t.me/tech_aks" target="_blank" className="flex items-center justify-between text-sm font-bold hover:text-[#05C3D4] transition-colors">
