...
export default function AdminBanners() {
    ...
    try {
      ...
      if (data.url) {
        setEditingBanner(prev =>
          prev ? { ...prev, image: data.url } : { image: data.url }
        );
  ...
  const linkOptions = [
    ...
    ...categories.map(c => ({
      label: `Каталог: ${c.name}`,
      value: `/catalog?cat=${c.slug}`,
    })),
      ...
      <div className="grid grid-cols-1 gap-6">
        {banners.length === 0 ? (
          ...
          banners.map(banner => (
            <div
              key={banner.id}
              className={`bg-white border rounded-2xl overflow-hidden flex flex-col md:flex-row ${!banner.active ? "opacity-60 grayscale" : "border-gray-200"}`}
            >
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="Например: Скидка 20% на все чехлы"
                    required
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-xl outline-none focus:border-[#05C3D4]"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-medium text-gray-700">
                  ...
                  <input
                    name="slug"
                    defaultValue={editingBanner.slug}
                    placeholder="skidka-20-cases"
                    required
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-xl outline-none focus:border-[#05C3D4]"
                  />
                </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  name="subtitle"
                  defaultValue={editingBanner.subtitle || undefined}
                  rows={2}
                  placeholder="Краткое описание акции для списка..."
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl outline-none focus:border-[#05C3D4] resize-none"
                />
              </div>

              <div className="space-y-1">
                <label className="text-sm font-medium text-gray-700">
                  Полный текст акции (HTML поддерживается)
                </label>
                <textarea
                  name="content"
                  defaultValue={editingBanner.content || undefined}
                  rows={6}
                  placeholder="Подробные условия, текст, списки..."
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl outline-none focus:border-[#05C3D4] font-mono text-sm"
                />
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
                ...
                <label
                  htmlFor="active"
                  className="text-sm font-bold text-[#0a0a0a] cursor-pointer flex items-center gap-2"
                >
                  <Eye size={16} className="text-[#05C3D4]" /> Акция активна и
                  видна на сайте
                </label>
              </div>
