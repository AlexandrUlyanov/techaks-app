# ТЗ и технический паспорт: интеграция Яндекс Доставки API 2.0 в Techaks
...
Интеграция проектируется по официальной документации Яндекс Доставки API 2.0:

- Quickstart
- расчёт предложений `offers/calculate`;
- создание, подтверждение, чтение и отмена заявки;
- получение фактического статуса заявки;
- GeoSuggest для подтверждения адреса.
...
- API-ошибки Яндекса не должны блокировать создание внутреннего заказа: при
  недоступном тарифе доставка передаётся менеджеру на ручной расчёт;
- повторный запрос создания заказа должен быть безопасным за счёт idempotency.

## 2.1 Реализованный production-контракт

- Тип доставки первой очереди: экспресс-доставка до двери по Пензе.
- Techaks хранит внутренний заказ независимо от состояния API перевозчика.
- Адрес выбирается через единый GeoSuggest и сохраняется в нормализованном виде.
- Точка отправления определяется складом комплектации заказа; если склад не выбран,
  используется настроенный склад доставки по умолчанию.
- Серверная котировка живёт 10 минут и связана с адресом, корзиной, ценами и складом
  через fingerprint.
- Цена перевозчика и цена для покупателя хранятся отдельно.
- Заявка создаётся фоново после полной оплаты и перехода заказа в статус
  `Передан в доставку`.
- Вызов перевозчика идемпотентен. Повторы выполняются очередью, а reconciliation
  восстанавливает пропущенные задачи.
- Статус доставки обновляется фоновым опросом API. Webhook не является единственной
  точкой отказа.
- Покупатель видит безопасный статус, ETA и данные назначенного курьера; технические
  ошибки и внутренние идентификаторы доступны только администратору.

## 2.2 Поддерживаемые возможности

| Возможность | Реализация |
|---|---|
| GeoSuggest и подтверждение адреса | backend proxy, один autocomplete в checkout |
| Расчёт до оформления | `offers/calculate`, серверная котировка |
| Несколько точек отправления | склад заказа или склад доставки по умолчанию |
| Вес и габариты | снимок упаковки из состава заказа и настроек по умолчанию |
| Количество и стоимость позиций | реальные order items, а не техническая заглушка |
| Наценка и субсидирование | процент, фиксированная наценка, субсидия, min/max |
| Бесплатная доставка | порог суммы корзины |
| Исключение товаров | `deliveryAllowed` и причина ограничения |
| Создание/подтверждение/отмена | API 2.0, только backend |
| Статус и ETA | фоновая синхронизация |
| Курьер | имя, телефон, автомобиль — только безопасные поля |
| Ошибки и повторы | durable queue, exponential retry, dead-letter state |
| Тест подключения | реальный dry-run расчёта с тестовым маршрутом |
| Тестовый/боевой режим | отдельные зашифрованные настройки окружения/БД |

Пункты выдачи, магистральная доставка и возвратная логистика подключаются как
отдельные методы доставки после подтверждения соответствующих тарифов в договоре
Яндекс Доставки. Они не маскируются под экспресс-доставку до двери.
