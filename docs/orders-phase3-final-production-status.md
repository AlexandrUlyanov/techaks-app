...
- этот документ фиксирует финальный статус именно rollout-фазы Orders на момент проверки;
- он не является live-inventory документом по текущему количеству заказов в production;
- после последующей очистки тестовых заказов historical references на `#14`, `#15`, `#18`, `#20` остаются частью этого snapshot.
